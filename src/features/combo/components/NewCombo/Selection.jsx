...
const Selection = () => {
    ...
    const {data, getAsyncProducts} = useProduct()
  ...
  return (
    <div>
      <div className="overflow-x-auto shadow-md sm:rounded-lg ">
          ...
          <tbody>
            {data.map((product) => (
              ...
              >
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      {...register("products", {
                        required: {
                          value: true,
                          message: "You must select at least a product",
                        },
                      })}
                      className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                    />
                ...
                >
                  {product.name}
                  {console.log(`${URL_STATIC}${product.image}`)}
                </th>
                <td className="p-4 w-32">
                  <img src={`${URL_STATIC}${product.image}`} alt="Apple Watch" />
                </td>
                <td className="py-4 px-6">{product.quantity}</td>
                <td className="py-4 px-6">{product.price}</td>
                <td className="py-4 px-6">
                  {product.variants.map((item) => item.k).join()}
                </td>
                <td className="py-4 px-6">
                  {product.variants
                    .map((item) => item.v)
        ...
        <nav className="flex justify-between items-center py-4 px-4 bg-gray-50 ">
        ...
        <div className="px-4">
            <Message text={errors?.products?.message}/>
        </div>
      </div>
    </div>
  );
