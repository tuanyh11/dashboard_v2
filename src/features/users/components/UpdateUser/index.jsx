...
const UpdateUser = () => {
  ...
  const {getAsyncUserById, updateAsyncUser , data} = useUser()
  ...
  const clearImg = () => {
    URL.revokeObjectURL(watch("newImage")?.[0]?.preview);
    setValue("newImage", "");
  };
        ...
        <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
          <div className="grid grid-cols-1 lg:grid-cols-0 gap-8">
            <div className="">
              <form
                action=""
                onSubmit={handleSubmit((data) => updateAsyncUser(data, nav))}
              >
                <Input
                  label={"email"}
                ...
                <div className="mt-10">
                  <InputFile
                    clearImg={clearImg}
                    isShowbutton={!!watch("newImage")?.[0]?.preview}
                    imageSrc={watch("newImage")?.[0]?.preview || URL_STATIC + watch("avatar")}
                    register={{
                      ...register("newImage", {
                        pattern: {
